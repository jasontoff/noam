...
│   ├── deploy-hosting.yml   #   → Firebase Hosting
...
- Changes under `fps-game/public/**` → Firebase Hosting redeploys (~35s)
...
If you ever need to deploy without CI:
...
- The server is authoritative — all hit detection, movement validation, and game state lives in `server.js`. The client only renders and sends input.
- `fps-game/public/game.js` is a single ~1600-line file. Sections are commented; search by feature (e.g. `// === SHOOTING ===`).
- Adding a new weapon: edit `WEAPONS` in `server.js` AND add a button in the weapon-select UI in `public/index.html`.
- Character and weapon models are in `public/models/`. They're GLB format loaded via Three.js GLTFLoader.

### Troubleshooting

- **Game loads but can't connect:** check Cloud Run service is up (`gcloud run services describe fps-game-server --region us-central1`). The hardcoded URL in `game.js` must match the Cloud Run URL.
